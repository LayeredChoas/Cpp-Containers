...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/14 19:09:15 by ayennoui          #+#    #+#             */
/*   Updated: 2021/03/04 16:00:30 by ayennoui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			 ...
			 class Alloc = std::allocator<ft::pair<const Key,T> >    // map::allocator_type
				 > class map
				 {
					 public:
						 ...
						 typedef ft::map_iter<value_type, value_type*, value_type&>						iterator;
						 typedef ft::map_iter< value_type, const value_type*, const value_type&>						const_iterator;
						 typedef ft::map_rev_iter<value_type, value_type*, value_type&>						reverse_iterator;
						 typedef ft::map_rev_iter< value_type, const value_type*, const value_type&>						const_reverse_iterator;
						 explicit map (const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()):c(comp),alloc(alloc){
							 _size = 0;
						 ...
						 template <class InputIterator>
							 map (InputIterator first, InputIterator last, const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()):c(comp),alloc(alloc){
								 _size = 0;
								 _head = new node<value_type>; 
								 _tail = new node<value_type>;
								 _head->next = _tail;
						 ...
						 ft::pair<iterator,bool> insert (const value_type& val){
							 ...
							 size_type count = 0;
							 while (cpy->next != NULL && count < _size)
							 {
								 if (cpy->_value.first == val.first)
						 ...
						 template <class Key1, class T1, class Compare1, class Alloc1>
							 friend bool operator!= ( const map<Key1,T1,Compare1,Alloc1>& lhs,
									 const map<Key1,T1,Compare1,Alloc1>& rhs ){return !(lhs == rhs);};
						 ...
						 template <class Key1, class T3, class Compare1, class Alloc1>
							 friend bool operator<= ( const map<Key1,T3,Compare1,Alloc1>& lhs,
									 const map<Key1,T3,Compare1,Alloc1>& rhs ){return !(rhs < lhs);};

						 template <class Key1, class T3, class Compare1, class Alloc1>
							 friend bool operator>  ( const map<Key1,T3,Compare1,Alloc1>& lhs,
									 const map<Key1,T3,Compare1,Alloc1>& rhs ){return ((rhs < lhs) && !(rhs == lhs));};

						 template <class Key1, class T3, class Compare1, class Alloc1>
							 friend bool operator>= ( const map<Key1,T3,Compare1,Alloc1>& lhs,
									 const map<Key1,T3,Compare1,Alloc1>& rhs ){return !(lhs < rhs);};
					 private:
